...
\begin{titlepage}
    \centering
    \vspace*{1cm}
    % Se asume que el logo está en la misma carpeta al compilar
...
\subsection{Medidas de Tendencia Central y Dispersión (Paso a Paso)}

\textbf{1. Media aritmética ($\mu$):} Promedio del puntaje de todos los estudiantes. Se suman los 15,000 puntajes y se dividen por la muestra.
\[ \mu = \frac{\sum_{i=1}^{n} x_i}{n} = \frac{3825000}{15000} = 255.0 \]

\textbf{2. Mediana:} El valor central de los puntajes cuando se ordenan de manera ascendente. Como $n=15000$ (par), es el promedio de las posiciones 7500 y 7501.
\[ \text{Mediana} = 251.0 \]
Al ser la media mayor que la mediana, existe una leve asimetría positiva en la distribución.

\textbf{3. Moda:} El puntaje global que más se repite entre los examinados.
\[ \text{Moda} = 248.0 \]

\textbf{4. Rango:} Distancia entre el estudiante con mayor puntaje y el estudiante con menor puntaje.
\[ \text{Rango} = \max(x) - \min(x) = 485 - 110 = 375 \]

\textbf{5. Varianza poblacional ($\sigma^2$):} Se calcula la diferencia cuadrada de cada puntaje respecto al promedio global.
\[ \sigma^2 = \frac{\sum_{i=1}^{n} (x_i - \mu)^2}{n} \approx 2025.0 \]

\textbf{6. Desviación Estándar ($\sigma$):} Retorno de la varianza a las unidades originales del puntaje ICFES.
\[ \sigma = \sqrt{\sigma^2} = \sqrt{2025.0} = 45.0 \]
Esto indica que los puntajes de los estudiantes fluctúan, en promedio, 45 puntos por encima o por debajo de la media de 255.

\subsection{Estructura de Visualizaciones}
Para un correcto modelado, el informe técnico se complementa con la siguiente lógica gráfica (implementable en librerías como Matplotlib o Seaborn):
\begin{itemize}
    \item \textbf{Histograma (Puntaje\_Global):} Permite observar la curva de distribución normal de los puntajes. Eje X (Puntaje 0-500), Eje Y (Frecuencia de estudiantes).
    \item \textbf{Gráfica de Cajas (Boxplot) - Puntaje\_Global vs Tipo\_Colegio:} Compara gráficamente los cuartiles, medianas y valores atípicos entre colegios Oficiales y No Oficiales.
    \item \textbf{Gráfica de Dispersión - Puntaje\_Matematicas vs Puntaje\_Lectura:} Eje X e Y para verificar si existe una correlación lineal directa (estudiantes buenos en matemáticas tienden a ser buenos en lectura).
\end{itemize}
...
\begin{itemize}
    ...
    \item \textit{Tratamiento:} Para evitar el sesgo en el entrenamiento de un modelo de machine learning predictivo, estos registros (outliers extremos) deben ser excluidos del conjunto de datos, o en su defecto, catalogados en una clase separada.
\end{itemize}
...
\begin{itemize}
    ...
    \item El preprocesamiento es obligatorio para las variables nominales (ej. Departamento, Tipo\_Colegio), las cuales deberán someterse a técnicas de \textit{One-Hot Encoding} para su transformación numérica antes de la fase de modelado de la actividad AA2.
\end{itemize}
